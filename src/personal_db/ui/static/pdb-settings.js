// Settings manage-list filter tabs (setup.html) + marketplace search
// (setup_browse.html). Both are pure client-side, no server round-trip:
//
//   - setup.html: each installed-source row carries data-status="ready" or
//     "attention" (see setup_runner.py's status_class -> the "● Ready" /
//     "Needs setup" chip already computed server-side). The All/Ready/Needs
//     attention tabs above the list just toggle row visibility.
//   - setup_browse.html: a search input filters the card grid by matching
//     against each card's data-search-text (title + description, lowercased
//     server-side) against the query, lowercased here. No debounce -- it's
//     a client-side array filter over ~20 cards, not a network call.
//
...
(function () {
  ...
  function applySourceFilter(tabsEl, filter) {
  ...
  function init() {
    ...
    if (tabsEl) applySourceFilter(tabsEl, "all");
  ...
  document.addEventListener("click", function (event) {
    ...
    if (!tabsEl) return;
    applySourceFilter(tabsEl, tabBtn.getAttribute("data-filter"));
  });

  document.addEventListener("input", function (event) {
    var target = event.target instanceof Element ? event.target : null;
    if (!target || !target.matches("[data-marketplace-search]")) return;
