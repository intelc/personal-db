// Runs in the page's MAIN world at document_start, before the creator SPA
// issues its own note-manager request.  It observes only the exact first-party
// search response and retains a normalized note projection: no request headers,
// cookies, query strings, or raw response bodies leave this page.
(() => {
  ...
  const imageUrl = (value) => {
    const candidate = typeof value === 'string' ? value
      : value && typeof value === 'object' ? first(value.url, value.url_default, value.urlDefault, value.origin_url, value.originUrl)
      : ''
    try {
  ...
  const normalize = (item) => {
    ...
    // IDs alone are not enough: this avoids treating unrelated nested objects
    // as notes when recursively walking the response.
    ...
    const hasNoteFields = Boolean(
      title || thumbnail || item.type || item.refTip || item.videoInfo || item.video_info
      || item.likes !== undefined || item.like_count !== undefined || item.likeCount !== undefined
    )
    ...
    return {
      ...
      share_count: count(first(item.share_count, item.shareCount, item.shared_count, interact.share_count, interact.shareCount, interact.shared_count)),
      source: 'creator-api',
    }
  }
  const publish = () => {
    state.rows = Array.from(rows.values())
    state.state = state.responseCount ? 'captured' : 'waiting'
  ...
  const exactSearch = (input) => {
    try {
      ...
      return ALLOWED_ENDPOINTS.has(`${url.origin}${url.pathname}`)
    } catch { return false }
  }
  const captureResponse = (response) => {
    if (!response?.ok) return
    response.clone().json().then(record).catch(() => { state.responseCount += 1; publish() })
  }

  const originalFetch = window.fetch
  if (typeof originalFetch === 'function') {
    window.fetch = function (...args) {
      const requested = exactSearch(args[0])
      if (requested) state.requestCount += 1
      const result = originalFetch.apply(this, args)
      if (requested) Promise.resolve(result).then(captureResponse).catch(() => {})
      return result
    }
  }
  ...
  XMLHttpRequest.prototype.open = function (method, url, ...args) {
    this.__personalDbXhsCreatorSearch = exactSearch(url)
  ...
  XMLHttpRequest.prototype.send = function (...args) {
    if (this.__personalDbXhsCreatorSearch) {
      ...
      this.addEventListener('loadend', () => {
        ...
        try { record(this.responseType === 'json' ? this.response : JSON.parse(this.responseText)) } catch { state.responseCount += 1; publish() }
      }, { once: true })
    }
